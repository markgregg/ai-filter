...
type BaseFieldDefinition = {
  ...
  label?: string;
  precedence: number;
  /** Minimum typed characters required before lookup-backed values are queried or matched. Defaults to `0`. */
  ...
  /** Optional predicate used by multi-value paste matching to validate a single pasted token. */
  pasteMatch?: (value: string, signal?: AbortSignal) => boolean | Promise<boolean>;
...
export type AiFilterProps = {
  ...
  /**
   * Enables local favorites tracking and limits favorite values per field.
   * When omitted, favorites are disabled.
   */
  maxFavorites?: number;
  /**
